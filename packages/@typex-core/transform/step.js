...
class Step {
  constructor(range) {
    ...
    this.lastAppliedPath = null
    this.lastInvertedPath = null
  }
...
export class SplitText extends Step {
  constructor(range, offset = range.offset) {
    super(range)
    this.offset = offset
  }
  applyAction (ops) {
    const res = ops.path.split(ops.offset)
    this.editor.selection.updatePoints(ops.path, ops.offset + 1, -ops.offset, res[1])
    return res
  }
  invertAction (ops) {
    mergeTextPath([ops.path, ops.path.nextSibling], this.editor)
  }
}
